...
import { FarcasterFollowerAddress, OnChainRecommendedUser } from "../interfaces/on-chain-recommended-user";
...
): Promise<FarcasterFollowerAddress[]> => {
  const farcasterFollowersResponse = await fetchAllPagesQuery<SocialFollowersData>(socialFollowersQuery, {
    user: address
  });
  return formatFarcasterFollowersData(
    farcasterFollowersResponse
      .flatMap(r => r.SocialFollowers?.Follower?.flatMap(f => f.followerAddress))
      .filter(Boolean),
    existingUsers
  );
};
