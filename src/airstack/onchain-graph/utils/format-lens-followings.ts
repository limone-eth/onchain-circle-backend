import { FollowLens, LensFollowingAddress, OnChainRecommendedUser } from "../interfaces/on-chain-recommended-user";

function formatLensFollowingsData(
  followings: LensFollowingAddress[],
  ...
  for (let i = 0; i < followings.length; i++) {
    ...
    if (existingUserIndex !== -1) {
      ...
      const follows: FollowLens = recommendedUsers[existingUserIndex]?.follows ?? {
        followingOnLens: false,
        followedOnLens: false
      };
