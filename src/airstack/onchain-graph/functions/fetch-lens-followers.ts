...
import { LensFollowerAddress, OnChainRecommendedUser } from "../interfaces/on-chain-recommended-user";
    ...
    SocialFollowers(
      ...
      Follower {
        followerAddress {
          ...
          mutualFollowing: socialFollowings(input: { filter: { identity: { _eq: $user }, dappName: { _eq: lens } } }) {
            Following {
              followingAddress {
                socials {
                  profileName
...
): Promise<LensFollowerAddress[]> => {
  const lensFollowersResponse = await fetchAllPagesQuery<SocialFollowersData>(socialFollowersQuery, {
    user: address
  });

  return formatLensFollowersData(
    lensFollowersResponse.flatMap(r => r.SocialFollowers?.Follower?.flatMap(f => f.followerAddress)).filter(Boolean),
    existingUsers
  );
};
